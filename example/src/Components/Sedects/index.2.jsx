      ...
      {check && subList.map((item, index) => {
        return item && (
          <section style={{marginTop: 10}}>
            <WeekComponent dataSource={item.subList} key={item.label} onChange={onChange} prefix={prefix}/>
          </section>
        ) || ''
      })}
    </article>
...
function WeekPicker ({dataSource, value={}, onChange = () => {}}) {
  ...
  return <WeekComponent dataSource={dataSource} value={value} onChange={handleChange} prefix={prefix.current}/>
}
