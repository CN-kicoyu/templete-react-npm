...
const Render = ({format, onChange, value, limitStart, limitEnd}) => {
  ...
  const disabledItem = (orientation, name) => (index) => {
    ...
    const res = isLeft ? (rightList[name] && index > Number(rightList[name])) || (limitStartList[name] && index < Number(limitStartList[name]))  : (leftList[name] && index < Number(leftList[name])) || (limitEndList[name] && index > Number(limitEndList[name]))
    switch(name) {
      case 'hour':
        return res
      case 'minute':
        return (rightList.hour && rightList.hour === leftList.hour || (limitStartList.hour && limitStartList.hour === leftList.hour) || (limitEndList.hour && limitEndList.hour === rightList.hour)) ? res : false
      case 'second':
        return (rightList.hour && rightList.hour === leftList.hour && rightList.minute && rightList.minute === leftList.minute) || (limitStartList.hour && limitStartList.hour === leftList.hour && limitStartList.minute && limitStartList.minute === leftList.minute) || (limitEndList.hour && limitEndList.hour === rightList.hour && limitEndList.minute && limitEndList.minute === rightList.minute) ? res : false
      default:
  ...
  useEffect(() => {
    ...
    if (limitStart && times[0] < limitStart) {
      setTimes(prev => {
        const temp = [...prev]
        temp[0] = moment(limitStart, format, true).add(1, 's').format(format);
        return temp
      })
    }
    onChange(times)
  ...
  return <div className="timesegment-wrap">
    <span className="timesegment-left">22222</span>
...
function TimeSegmentPicker(props) {
  ...
  const handlerProps = (index) => {
    ...
    if (index < props.value.length - 1) {
      ...
      return nextList && ({ limitEnd: nextList[0] ? nextList[0] : nextList[1] })
    }
    return {}
  }
