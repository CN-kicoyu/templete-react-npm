...
const Render = ({format, onChange, value, canCross}) => {
...
function TimeSegmentPicker(props) {
  const {format, canCross, ...rest} = props
  console.log(props.value, '----->>>>>>>#3333')

  return (
    <MutilEdit {...rest} subValue={[]}>
      <Render format={format} onChange={props.onChange} canCross={canCross}/>
    </MutilEdit>
  )
}
